...
* トラフィックミラーリングは記録しません。
* ARPパケットは記録しません。
* インスタンスを含む物理装置またはネットワークサービスの物理装置で一時的なネットワーク混雑によって発生する`DROP`は収集対象ではありません。
...
* ロードバランサーに設定されたIPACLによってブロックされたパケットは、現在`DROP`ではなく`ACCEPT`に分類されます。DROP`の表記区分は後日サポート予定です。
* ロードバランサーへのアクセスを試みるパケット、ロードバランサーとメンバー間のパケットに加えて、ヘルスチェック用のパケットもあわせて収集します。
...
* ユーザーが明示的に`DROP`を設定できるサービスではないため、 `DROP`はサポートしません。
