...
* 트래픽 미러링은 기록하지 않습니다.
* ARP 패킷은 기록하지 않습니다.
* 인스턴스를 포함하는 물리적 장비 또는 네트워크 서비스의 물리적 장비에서 일시적인 네트워크 혼잡에 의하여 내부 장비에서 발생하는 `DROP`은 수집 대상이 아닙니다.
...
### 로드 밸런서에 플로우 로그를 지정사여 사용 시 유의 사항
...
* 해당 서비스에 연결된 플로우 로그는 `direction` 필드가 `not_supported`로 표기됩니다.
* 해당 서비스에 연결된 플로우 로그는 `연결 수립 패킷만 수집(connection setup only)` 옵션의 영향을 받지 않으며, 연결 상태와 관계없이 모든 패킷을 수집합니다.
